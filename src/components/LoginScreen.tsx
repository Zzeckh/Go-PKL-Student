...
import {
  ArrowRight, Mail, Lock, Eye, EyeOff, AlertCircle, Loader2, Check, LogIn, MapPin, BookOpen, FileCheck,
} from 'lucide-react';
...
/* ── Sapaan multibahasa: CJK → Arab → Rusia → India → Inggris → Indonesia ── */
const GREETINGS = [
  '欢迎',               // Cina
  'ようこそ',           // Jepang
  '환영합니다',         // Korea
...
/* ══════════════════════════════════════════════════════
   SPLASH (1,5s) → LOADING (2,5s) → START → LOGIN
   ══════════════════════════════════════════════════════ */
export const LoginScreen: React.FC<LoginScreenProps> = ({ onSuccess }) => {
  ...
  /* ── Choreography: splash 1,5s → loading 2,5s → start ── */
  useEffect(() => {
    if (stage === 'splash') {
    ...
    if (stage === 'loading') {
      ...
      const t = setTimeout(() => setStage('start'), 2500);
  ...
  return (
    <div className="relative min-h-screen bg-white overflow-hidden">
      ...
      {/* ═══ STAGE: LOADING (2,5 dtk) — sapaan multibahasa + progress bar ═══ */}
      {stage === 'loading' && (
        <div
          key="loading"
      ...
      {/* ═══ STAGE: START — hero + chips fitur + CTA ═══ */}
      {stage === 'start' && (
        <div key="start" className="absolute inset-0 z-10 flex flex-col rise-in">
          ...
          <div className="p-6 pt-0">
            ...
            <p className="text-center text-[11px] font-semibold text-white/50 mt-3">
              Masuk dengan akun yang dibuat admin sekolah
            </p>
          </div>
        </div>
      )}
      ...
      {stage === 'login' && (
        <>
          ...
          <div key="form" className="absolute inset-x-0 bottom-0 top-[calc(30%+72px)] z-10 overflow-y-auto rise-in">
