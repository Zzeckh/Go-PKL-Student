...
const GREETINGS = [
  '欢迎', 'ようこそ', '환영합니다', 'أهلاً وسهلاً',
  'Добро пожаловать', 'स्वागत है', 'Welcome', 'Selamat Datang',
];
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onSuccess, skipIntro = false }) => {
  ...
  const splashView = (
    <div className="w-full h-full flex flex-col items-center justify-center">
      ...
      <h1 className="text-white text-4xl font-extrabold mt-7 tracking-tight splash-item" style={{ animationDelay: '250ms' }}>Go-PKL</h1>
      <div className="w-10 h-[3px] bg-white/40 rounded-full mt-3 splash-divider" style={{ animationDelay: '400ms' }} />
      <p className="text-white/60 text-sm font-semibold mt-3 splash-item" style={{ animationDelay: '450ms' }}>Portal Magang Siswa</p>
      <p className="absolute bottom-8 text-[10px] font-semibold text-white/40 splash-item" style={{ animationDelay: '600ms' }}>© 2026 Go-PKL</p>
    </div>
  );
    ...
    <div className="w-full h-[70%] flex flex-col items-center justify-center px-8">
      ...
      <div className="w-full max-w-[264px] mt-6">
        <div className="h-2 bg-mist rounded-full overflow-hidden">
          <div className="h-full bg-navy rounded-full transition-[width] duration-100 ease-linear" style={{ width: `${percent}%` }} />
        </div>
      ...
      <div className="flex items-center gap-2 mt-6">
        {STEPS.map(s => {
          ...
          return (
            <span key={s.label} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[10px] font-bold transition-all duration-300 ${
              active ? 'bg-navy border-navy text-white shadow-sm scale-105' : 'bg-white border-mist text-navy/40'
            }`}>
  ...
  const startView = (
    <div className="w-full h-full flex flex-col items-center">
      <div className="flex-1 flex flex-col items-center justify-center px-8 text-center pt-24">
        <h1 className="splash-item text-white text-[32px] font-extrabold tracking-tight mt-6" style={{ animationDelay: '200ms' }}>Ayo Mulai!</h1>
        <div className="splash-item w-10 h-[3px] bg-white/40 rounded-full mt-4" style={{ animationDelay: '300ms' }} />
        <p className="splash-item text-white/60 text-sm font-semibold mt-4 leading-relaxed" style={{ animationDelay: '350ms' }}>
          Masuk dan mulai langkah magangmu hari ini.
        </p>
      </div>
      <div className="splash-item w-full px-6 pb-[calc(24px+env(safe-area-inset-bottom))]" style={{ animationDelay: '450ms' }}>
        <button onClick={() => goTo('login')} className="w-full bg-white text-navy rounded-full py-4 text-sm font-extrabold shadow-lg shadow-black/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
          Masuk dengan Akun Sekolah <ArrowRight className="w-4 h-4" />
        </button>
        <p className="text-center text-[11px] font-semibold text-white/50 mt-3">Butuh bantuan? Hubungi tim Hubin sekolahmu.</p>
      </div>
    </div>
  );
      ...
      <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
        ...
        <div>
          ...
          <div className="flex items-center gap-2.5 border-b border-mist focus-within:border-steel transition-colors py-2.5">
            ...
            <button type="button" onClick={() => setShowPassword(!showPassword)}
              className="text-navy/40 hover:text-navy transition-colors" aria-label="Tampilkan password">
        ...
        <div className="flex items-center justify-between">
          <button type="button" onClick={() => setRemember(!remember)} className="flex items-center gap-2">
            ...
            }`}>{remember && <Check className="w-3 h-3 text-white" />}</span>
            <span className="text-xs font-semibold text-navy/70">Ingat saya</span>
          </button>
          <button type="button" className="text-xs font-bold text-steel hover:underline">Lupa password?</button>
        </div>
        {error && (
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? (<><Loader2 className="w-4 h-4 animate-spin" /> Memeriksa...</>) : (<><LogIn className="w-4 h-4" /> Masuk</>)}
        </button>
        <p className="text-center text-[11px] font-semibold text-navy/50">Belum punya akun? Akun dibuat oleh admin sekolah.</p>
      </form>
    </div>
  );
  ...
  return (
    <div className="relative min-h-screen bg-white overflow-hidden">
      ...
      <div
        ...
        style={{ animationDelay: loginDelay }}
      >
        {VIEWS[stage]}
      </div>
    </div>
